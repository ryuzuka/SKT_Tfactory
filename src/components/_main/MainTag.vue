  ...
  methods: {
    ...
    getZoneRecommend () {
      ...
      MYTAG.getZoneRecommend().then(result => {
        ...
        this.recommendList.forEach((recommend, recommendIndex) => {
          if (!this.recommendListByFloor[recommend.ZONE_FLOOR]) {
            this.recommendListByFloor[recommend.ZONE_FLOOR] = []
          }
          recommend.INDEX = recommendIndex
          this.recommendListByFloor[recommend.ZONE_FLOOR].push(recommend)
        })
        this.$loading.stop()
      }).catch((err) => {
        console.log(err)
    ...
    getMyHistory (date) {
      MYTAG.getTimelines().then(result => {
        ...
        result.TIMELINE_LIST.forEach((history) => {
          history.REG_YMD = this.$moment(history.REG_YMDT).format('YY.MM.DD')
  ...
  watch: {
    isLogin: function () {
      if (this.isLogin) {
        ...
        this.today = this.$moment().format('YY.MM.DD')
        this.userInfo = this.$cookies.get('MY_INFO')
        this.nickname = this.userInfo.NICKNAME
        this.selectedNumber = this.userInfo.MDN
