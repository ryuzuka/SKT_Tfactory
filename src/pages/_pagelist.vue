<template>
  <div class="page__pagelist app__SKT">
    <div id="wrapper">
      <header id="header">
        <h1 class="title">Pagelist</h1>
        <div class="status">최초공유 / 수정전달</div>
      </header>
      <main id="container">
        <div class="box__pagelist">

          <h2 class="pagelist-title">Guide</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/guide'>UI Elements 가이드</router-link>
              <div class="date"><span></span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">comm</h2>
          <ul>
            <!--            <li class="pagelist-item"><router-link class="link" to='/confirm/allmenulogin'>전체메뉴 - 로그인 (확인용)</router-link><div class="date"><span>3.5</span></div></li>-->
            <li class="pagelist-item">
              <router-link class="link" to='/error/400'>400 에러</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/error/500'>500 에러</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/error/check'>시스템점검</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/familysite'>패밀리 사이트</router-link>
              <div class="date"><span>3.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/corpinfo'>사업자 정보</router-link>
              <div class="date"><span>3.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/setting'>설정</router-link>
              <div class="date"><span>4.9</span><span>11.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/settinglogout'>설정 - 로그아웃</router-link>
              <div class="date"><span>4.9</span></div>
            </li>
            <!--            <li class="pagelist-item">-->
            <!--              <router-link class="link" to='/member'>T아이디 회원정보 관리</router-link>-->
            <!--              <div class="date"><span>4.9</span></div>-->
            <!--            </li>-->
            <li class="pagelist-item">
              <router-link class="link" to='/terms'>약관 상세</router-link>
              <div class="date"><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/leave'>회원 탈퇴</router-link>
              <div class="date"><span>7.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/leave/complete'>회원 탈퇴 완료</router-link>
              <div class="date"><span>7.15</span></div>
            </li>

            <li class="pagelist-item">
              <router-link class="link" to='/lineinfo'>T Factory 회선 정보 관리</router-link>
              <div class="date"><span>11.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/combine'>회원 정보 통합</router-link>
              <div class="date"><span>11.06</span><span>12.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/join'>T Factory 회원 가입</router-link>
              <div class="date"><span>11.06</span>12.15</div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/policy/advg'>광고성 정보 수신 동의</router-link>
              <div class="date"><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/policy/bio'>생체(얼굴)정보 수집이용 동의</router-link>
              <div class="date"><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/policy/privacy'>임시회원 개인정보 수집이용 동의 (필수동의)
              </router-link>
              <div class="date"><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/policy/privacyopt'>개인정보 수집이용 동의 (선택동의)</router-link>
              <div class="date"><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/policy/service'>서비스 이용약관_인스턴트바이 반영</router-link>
              <div class="date"><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/qrcode'>QR CODE</router-link>
              <div class="date"><span>21.5.11</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">main 9월</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/main/store'>메인 - STORE</router-link>
              <div class="date"><span>7.31</span><span>2.1</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/main/program'>메인 - PROGRAM</router-link>
              <div class="date"><span>7.31</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/main/mytag'>메인 - MY TAG</router-link>
              <div class="date"><span>7.31</span><span>1.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/main/menu'>메인 - MENU</router-link>
              <div class="date"><span>8.21</span><span>1.8</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mainlogin'>메인 로그인 유도</router-link>
              <div class="date"><span>4.16</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mainappdown'>메인 모바일앱 유도</router-link>
              <div class="date"><span>4.16</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">My Tag</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/mytag/login'>로그인 전 - T아이디 회원인 경우</router-link>
              <div class="date"><span>7.31</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mytag/certify'>로그인 전 - T아이디 회원이 아닌경우</router-link>
              <div class="date"><span>7.31</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mytag/reissue'>인증번호 재발급</router-link>
              <div class="date"><span>7.31</span><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/mytag/servey'>My Tag 만들기</router-link>
              <div class="date"><span>7.31</span><span>9.8</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mytag/mms'>MMS 접근</router-link>
              <div class="date"><span>9.8</span><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/mytag/additional'>부가서비스</router-link>
              <div class="date"><span>9.8</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">sso</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/sso/login'>로그인</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sso/kiosk'>kiosk 로그인</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sso/success'>kiosk 로그인 성공</router-link>
              <div class="date"><span>3.2</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sso/certify'>빠른 체크인</router-link>
              <div class="date"><span>3.23</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">prd</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/prd/000003972'>상품상세/아이폰11</router-link>
              <div class="date"><span>2.24</span><span>4.14</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/productbuttoncase'>상품상세 - 상담 신청하기 (확인용)</router-link>
              <div class="date"><span>6.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience'>상품상세/체험존</router-link>
              <div class="date"><span>9.18</span><span>12.4</span></div>
            </li>
          </ul>
          <h2 class="pagelist-title">진행중인 프로그램 상세 (하드코딩)</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program1'>이번주 속도왕은 바로 나!<br>팩토리 속도왕</router-link>
              <div class="date"><span>12.9</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program2'>갤럭시S21 'TMI'</router-link>
              <div class="date"><span>12.9</span><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program3'>e-스포츠에 대한 모든것<br>with 프로게이머</router-link>
              <div class="date"><span>12.9</span><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program4'>갤럭시로 한 곡 뚝딱!</router-link>
              <div class="date"><span>12.7</span><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program5'>인싸 영상 만들기</router-link>
              <div class="date"><span>12.9</span><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program6'>가장 일상적인 이상적 카메라, 당신의 휴대폰</router-link>
              <div class="date"><span>12.9</span><span>12.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program7'>T Factory 숨은 아이폰 덕후를 찾아라! 덕후크루와 한판 승부!</router-link>
              <div class="date"><span>12.9</span><span>12.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program8'>나상현씨밴드<br>T Factory 콘서트의 시작</router-link>
              <div class="date"><span>12.9</span><span>3.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program9'>페퍼톤스</router-link>
              <div class="date"><span>12.9</span><span>12.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program10'>브로콜리 너마저</router-link>
              <div class="date"><span>12.9</span><span>3.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program11'>갤럭시 스튜디오 [Galaxy S21 체험존] T팩토리에서 Galaxy Studio를
                만나보세요!
              </router-link>
              <div class="date"><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program12'>Love Galaxy @T Factory<br>갤럭시S21 출시기념 특별 이벤트, 오직 T
                Factory에서만!
              </router-link>
              <div class="date"><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program13'>이달의 악기는?<br>클래식 악기 릴레이 체험 전시</router-link>
              <div class="date"><span>1.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program14'>SAVE THE MOMENT</router-link>
              <div class="date"><span>2.26</span><span>3.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program15'>Welcome to ⓥ Studio!</router-link>
              <div class="date"><span>2.26</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program16'>나의 ⓥTYPE 찾기</router-link>
              <div class="date"><span>2.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program17'>너도 이제 크리에이터</router-link>
              <div class="date"><span>3.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program18'>4월의 덕콘 | 안예은</router-link>
              <div class="date"><span>3.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program19'>4월의 덕콘 | 불고기디스코</router-link>
              <div class="date"><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program20'>4월의 덕콘 | 후추스</router-link>
              <div class="date"><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program21'>4월의 덕콘 | 페퍼톤스</router-link>
              <div class="date"><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program22'>Xbox 컨트롤러 TO-GO</router-link>
              <div class="date"><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program23'>오큘러스 퀘스트 2 한정 판매</router-link>
              <div class="date"><span>4.7</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program24'>Stage Q 사전예약 이벤트</router-link>
              <div class="date"><span>4.16</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program25'>5월의 덕콘 | SURL</router-link>
              <div class="date"><span>4.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program26'>5월의 덕콘 | 신지훈</router-link>
              <div class="date"><span>4.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program27'>5월의 덕콘 | 형태</router-link>
              <div class="date"><span>4.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program28'>5월의 덕콘 | 프롬</router-link>
              <div class="date"><span>4.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program29'>5월의 덕콘 | 블루파프리카</router-link>
              <div class="date"><span>4.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program30'>Q레벨 테스트</router-link>
              <div class="date"><span>4.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program31'>Stage Q 참여 인증 이벤트</router-link>
              <div class="date"><span>4.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program32'>도전하라 stage Q</router-link>
              <div class="date"><span>4.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program33'>퀀텀2를 잡아라</router-link>
              <div class="date"><span>5.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program34'>6월의 덕콘 | 제8극장</router-link>
              <div class="date"><span>5.20</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program35'>6월의 덕콘 | 데이먼스 이어</router-link>
              <div class="date"><span>5.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program36'>6월의 덕콘 | 에이트레인</router-link>
              <div class="date"><span>5.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program37'>6월의 덕콘 | 빌리 어코스티</router-link>
              <div class="date"><span>5.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program38'>5월의 덕콘</router-link>
              <div class="date"><span>5.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program39'>6월의 덕콘</router-link>
              <div class="date"><span>5.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program40'>7월의 덕콘 | HippoPotamic</router-link>
              <div class="date"><span>7.05</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program41'>7월의 덕콘 | 딕펑스</router-link>
              <div class="date"><span>7.05</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program42'>7월의 덕콘 | 윤현상</router-link>
              <div class="date"><span>7.05</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program43'>7월의 덕콘 | 위아더나잇</router-link>
              <div class="date"><span>7.05</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program44'>7월의 덕콘</router-link>
              <div class="date"><span>7.05</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program45'>Unfold Your Summer</router-link>
              <div class="date"><span>8.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program46'>덕콘 Special | ADOY</router-link>
              <div class="date"><span>8.20</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program47'>루시드폴의 사운드 제주</router-link>
              <div class="date"><span>8.20</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program48'>리얼리티 우주 감각 전시 미퓨의 방</router-link>
              <div class="date"><span>8.20</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program49'>곰손도 가능! 에어팟 가죽케이스 만들기</router-link>
              <div class="date"><span>9.14</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program50'>"JUST PLAY. iPhone13. 활용 꿀팁 마스터 (60min)"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program51'>"촬영부터 편집까지! 스마트폰으로 브이로그 만들기"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program52'>"IT와 미술의 만남 아이패드로 쉽게 드로잉하기"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program53'>"인스타용 느낌있는 사진 찍기"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program54'>"Thanks 티팩 Day!"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program55'>"just play iphone13"</router-link>
              <div class="date"><span>9.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program56'>"ADOY를 영화처럼 담아줄 팬분을 찾습니다."</router-link>
              <div class="date"><span>10.8</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program57'>"하시시박의 일주일 Ordinary week, extraordinary iPhone13"</router-link>
              <div class="date"><span>10.8</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program58'>10월의 덕콘 | THAMA</router-link>
              <div class="date"><span>10.13</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program59'>10월의 덕콘 | 허성현(Huh!)</router-link>
              <div class="date"><span>10.13</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program60'>10월의 덕콘</router-link>
              <div class="date"><span>10.13</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program61'>11월의 덕콘 | 김제형</router-link>
              <div class="date"><span>10.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program62'>11월의 덕콘 | 너드커넥션</router-link>
              <div class="date"><span>10.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program63'>11월의 덕콘</router-link>
              <div class="date"><span>10.26</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program64'>Apple매니아의 완성, SK텔레콤</router-link>
              <div class="date"><span>11.17</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program65'>12월의 덕콘 | 김오키새턴발라드</router-link>
              <div class="date"><span>11.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program66'>12월의 덕콘 | ADOY</router-link>
              <div class="date"><span>11.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program67'>12월의 덕콘 | 아메바컬쳐 (with SOLE, THAMA, Huh!, snzae)</router-link>
              <div class="date"><span>11.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program68'>12월의 덕콘</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program69'>12월의 덕콘 | 불고기디스코</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program70'>12월의 덕콘 | 크라잉넛</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program71'>12월의 덕콘 | 최엘비</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program72'>12월의 덕콘 | 브로콜리너마저</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program73'>12월의 덕콘 | 디핵</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program74'>12월의 덕콘 | 던밀스</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program75'>12월의 덕콘 | 한올</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program76'>12월의 덕콘 | 프롬,후추스</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program77'>12월의 덕콘 | 저드</router-link>
              <div class="date"><span>12.07</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program78'>12월의 덕콘 | 페퍼톤스</router-link>
              <div class="date"><span>12.07</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program79'>12월의 덕콘 | PL</router-link>
              <div class="date"><span>12.07</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program80'>12월의 덕콘 | 나상현씨밴드</router-link>
              <div class="date"><span>12.07</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program81'>체험형 음악전시 음악을 즐기는 N가지 방법</router-link>
              <div class="date"><span>12.03</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program82'>DJ 포퍼먼스 음악을 즐기는 N가지 방법</router-link>
              <div class="date"><span>12.10</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program83'>메타버스에서 나의 아티스트를 만나다 | 던밀스 X ifland 팬미팅</router-link>
              <div class="date"><span>12.10</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program84'>인기 게임 정복하기</router-link>
              <div class="date"><span>12.14</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program85'>F.Y.i OFFICE : FOR YOUR iNTERNSHIP</router-link>
              <div class="date"><span>12.14</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program86'>1월의 덕콘</router-link>
              <div class="date"><span>12.22</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program87'>1월의 덕콘 | 모브닝</router-link>
              <div class="date"><span>12.22</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program88'>1월의 덕콘 | 터치드</router-link>
              <div class="date"><span>12.22</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program89'>1월의 덕콘 | 안녕바다</router-link>
              <div class="date"><span>12.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program90'>1월의 덕콘 | W24</router-link>
              <div class="date"><span>12.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program91'>1월의 덕콘 | 언오피셜보이</router-link>
              <div class="date"><span>12.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program92'>1월의 덕콘 | 너드커넥션</router-link>
              <div class="date"><span>12.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program93'>1월의 덕콘 | 데이브레이크</router-link>
              <div class="date"><span>12.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program94'>"메타버스에서 나의 아티스트를 만나다 | W24 X ifland 팬미팅"</router-link>
              <div class="date"><span>1.10</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program95'>"메타버스에서 나의 아티스트를 만나다 | 언오피셜보이 X ifland 팬미팅"</router-link>
              <div class="date"><span>1.10</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program96'>[T Factory X T1] LoL Class 런칭기념 l 'Piglet' 채광진 코치 초청 Class</router-link>
              <div class="date"><span>1.14</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program97'>&#60;NEXT 트레이서&#62;</router-link>
              <div class="date"><span>1.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program98'>2월의 덕콘</router-link>
              <div class="date"><span>2.3</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program99'>2월의 덕콘 | 안녕하신가영</router-link>
              <div class="date"><span>2.3</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program100'>2월의 덕콘 | 데자부 그룹</router-link>
              <div class="date"><span>2.3</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program101'>3월의 덕콘</router-link>
              <div class="date"><span>2.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program102'>3월의 덕콘 | 잭킹콩</router-link>
              <div class="date"><span>2.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program103'>3월의 덕콘 | 솔루션스</router-link>
              <div class="date"><span>2.28</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program104'>T Factory x FLO 덕콘</router-link>
              <div class="date"><span>3.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program105'>T Factory POP-UP Exhibition</router-link>
              <div class="date"><span>3.15</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program106'>LCK 스프링결승전 응원전</router-link>
              <div class="date"><span>3.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program107'>4월의 덕콘</router-link>
              <div class="date"><span>3.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program108'>4월의 덕콘 | 손태진</router-link>
              <div class="date"><span>3.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program109'>4월의 덕콘 | 전진희</router-link>
              <div class="date"><span>3.25</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program110'>T대학 우주 축제</router-link>
              <div class="date"><span>3.28</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program111'>THECON X University</router-link>
              <div class="date"><span>3.28</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program112'>POP-UP Exhibition X University</router-link>
              <div class="date"><span>3.28</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program114'>5월의 덕콘</router-link>
              <div class="date"><span>4.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program116'>5월의 덕콘 | 장희진</router-link>
              <div class="date"><span>4.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program116'>5월의 덕콘 | 브랜드뉴차일드/미스몰리</router-link>
              <div class="date"><span>4.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/experience/program116'>5월의 덕콘 | 라쿠나</router-link>
              <div class="date"><span>4.27</span></div>
            </li>
	          <li class="pagelist-item">
		          <router-link class="link" to='/experience/program200'>6월의 덕콘 | 세이수미</router-link>
		          <div class="date"><span>5.26</span></div>
	          </li>
	          <li class="pagelist-item">
		          <router-link class="link" to='/experience/program201'>6월의 덕콘 | 요조</router-link>
		          <div class="date"><span>5.26</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program202'>6월의 덕콘</router-link>
		          <div class="date"><span>5.30</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program203'>POP-UP Exhibition X You</router-link>
		          <div class="date"><span>6.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program204'>7월의 덕콘</router-link>
		          <div class="date"><span>6.22</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program205'>7월의 덕콘 | 해서웨이</router-link>
		          <div class="date"><span>6.22</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program206'>7월의 덕콘 | 소수빈</router-link>
		          <div class="date"><span>6.22</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program207'>공개방송 방청 | 랄랄,김진주</router-link>
		          <div class="date"><span>7.15</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program208'>8월의 덕콘</router-link>
		          <div class="date"><span>8.19</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program209'>8월의 덕콘 | 실리카겔</router-link>
		          <div class="date"><span>8.19</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program210'>8월의 덕콘 | 차세대</router-link>
		          <div class="date"><span>8.26</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program211'>공개방송 방청 | 플로리다 프로젝트</router-link>
		          <div class="date"><span>8.13</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program212'>Z Flip 4와 함께하는 나의 빛나는 순간들</router-link>
		          <div class="date"><span>8.13</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program213'>With Z Flip 4 & XEXYMIX</router-link>
		          <div class="date"><span>8.16</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program214'>With Z Flip 4 & XEXYMIX | 윤채형&윤원형</router-link>
		          <div class="date"><span>8.20</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program215'>With Z Flip 4 & XEXYMIX | 신지수</router-link>
		          <div class="date"><span>8.27</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program216'>With Z Flip 4 & XEXYMIX | 오윤하</router-link>
		          <div class="date"><span>9.03</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program217'>With Z Flip 4 & XEXYMIX | 고유라</router-link>
		          <div class="date"><span>9.10</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program218'>With Z Flip 4 & XEXYMIX | 박소리</router-link>
		          <div class="date"><span>9.17</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program219'>With Z Flip 4 & XEXYMIX | 최유진</router-link>
		          <div class="date"><span>9.24</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program220'>With Z Flip 4 & XEXYMIX | 서홍</router-link>
		          <div class="date"><span>9.29</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program221'>LCK 서머 플레이오프 응원전</router-link>
		          <div class="date"><span>8.21</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program222'>9월의 덕콘</router-link>
		          <div class="date"><span>9.16</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program223'>9월의 덕콘 | PATZ</router-link>
		          <div class="date"><span>9.16</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program224'>9월의 덕콘 | 유키카</router-link>
		          <div class="date"><span>9.23</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program225'>T Factory X ifland | 메타홍대 뮤직투어 :후추스의 인디벌스</router-link>
		          <div class="date"><span>9.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program226'>10월의 덕콘</router-link>
		          <div class="date"><span>10.04</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program227'>10월의 덕콘 | 맥거핀</router-link>
		          <div class="date"><span>10.04</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program228'>10월의 덕콘 | Youth Hostel</router-link>
		          <div class="date"><span>10.04</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program229'>[Crew Class for New 0’s] iPhone 14/Pro 완전정복</router-link>
		          <div class="date"><span>10.05</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program230'>[Crew Class for New 0’s] VLOG 만들기 (feat.시네마틱모드)</router-link>
		          <div class="date"><span>10.05</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program231'>[Crew Class for New 0’s] 릴스 만들기 (feat.액션모드)</router-link>
		          <div class="date"><span>10.05</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program232'>T Factory for NEW 0s | T Factory의 모든 것을 NEW 0s답게 즐겨보세요!</router-link>
		          <div class="date"><span>10.07</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program233'>Arcade for NEW 0s | 게임에 도전해봐! 두려울 거 없잖아!</router-link>
		          <div class="date"><span>10.07</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program234'>Untact Shop for NEW 0s | 대면보다 비대면이 편한 NEW 0s를 위한</router-link>
		          <div class="date"><span>10.07</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program235'>T Factory x FLO | 최애 플레이리스트 첫 공개방송 방청 신청</router-link>
		          <div class="date"><span>10.12</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program236'>T Factory X T1 | 롤드컵 8강 응원전</router-link>
		          <div class="date"><span>10.17</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program237'>T Factory | DRX 무릎 철권7 for NEW 0s</router-link>
		          <div class="date"><span>10.21</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program238'>T Factory | DRX 무릎 철권7 for NEW 0s 대회 참가 선수 신청</router-link>
		          <div class="date"><span>10.21</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program239'>T Factory | DRX 무릎 철권7 for NEW 0s 대회 직관 신청</router-link>
		          <div class="date"><span>10.21</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program240'>T Factory X T1 | 롤드컵 4강 응원전</router-link>
		          <div class="date"><span>10.24</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program241'>11월의 덕콘</router-link>
		          <div class="date"><span>10.27</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program242'>11월의 덕콘 | 크리스탈티</router-link>
		          <div class="date"><span>10.27</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program243'>11월의 덕콘 | 미스피츠</router-link>
		          <div class="date"><span>10.27</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program244'>T Factory X T1 | 롤드컵 파이널 응원전</router-link>
		          <div class="date"><span>10.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program245'>나만의 디지털 메이트를 만나는 곳 A. Factory</router-link>
		          <div class="date"><span>11.11</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program246'>수능 이벤트 | 수능 끝! 얘드라 혜택 챙겨!</router-link>
		          <div class="date"><span>11.11</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program247'>수능 이벤트 | 얘드라 콘서트 챙겨!</router-link>
		          <div class="date"><span>11.11</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program248'>수험생을 위한 &lt;스페셜 덕콘&gt; | 옥상달빛</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program249'>수험생을 위한 &lt;스페셜 덕콘&gt; | 데자부그룹</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program250'>수험생을 위한 &lt;스페셜 덕콘&gt; | 구만</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program251'>수험생을 위한 &lt;스페셜 덕콘&gt; | 소란</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program252'>수험생을 위한 &lt;스페셜 덕콘&gt; | 치즈</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program253'>수능 이벤트 | 얘드라 책 덮고 클래스 챙겨!</router-link>
		          <div class="date"><span>11.11</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program254'>수능 이벤트 | 펀칭니들 클래스</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program255'>수능 이벤트 | 자이언트 얀 가방 클래스</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program256'>수능 이벤트 | 크리스마스 리스 클래스</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program257'>수능 이벤트 | 뷰티클래스</router-link>
		          <div class="date"><span>11.14</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program258'>수능 이벤트 | 얘드라 새로 바꿔야지, 애플 할인 챙겨!</router-link>
		          <div class="date"><span>11.11</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program259'>12월의 덕콘</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program260'>12월의 덕콘 | 쏜애플</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program261'>12월의 덕콘 | 데이브레이크</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program262'>12월의 덕콘 | 유다빈밴드</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program263'>12월의 덕콘 | SURL</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program264'>12월의 덕콘 | LUCY</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program265'>12월의 덕콘 | ADOY</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program266'>12월의 덕콘 | 홍이삭</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program267'>12월의 덕콘 | 크리스탈티</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program268'>12월의 덕콘 | 윤석철트리오</router-link>
		          <div class="date"><span>12.02</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program269'>1월의 덕콘</router-link>
		          <div class="date"><span>01.13</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program270'>1월의 덕콘 | 김수영</router-link>
		          <div class="date"><span>01.13</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program271'>1월의 덕콘 | 후추스 X Sofia Mills  </router-link>
		          <div class="date"><span>01.20</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program272'>1월의 덕콘 | 맥거핀</router-link>
		          <div class="date"><span>01.27</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program273'>2월의 덕콘 with MPMG</router-link>
		          <div class="date"><span>01.31</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program274'>2월의 덕콘 | MPMG WEEK</router-link>
		          <div class="date"><span>01.31</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program275'>2월의 덕콘 | 노리플라이</router-link>
		          <div class="date"><span>01.31</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program276'>2월의 덕콘 | 제이유나</router-link>
		          <div class="date"><span>01.31</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program277'>T Factory x T1 Will to Win 스프링시즌 Event</router-link>
		          <div class="date"><span>02.07</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program278'>Will to Win T1 티게더 뷰잉파티 Event</router-link>
		          <div class="date"><span>02.07</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program279'>Galaxy S23 POP UP Store</router-link>
		          <div class="date"><span>02.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program280'>Galaxy S23 POP UP Store 현장 이벤트</router-link>
		          <div class="date"><span>02.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program281'>Just paused, 우리가 담긴 순간들</router-link>
		          <div class="date"><span>03.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program282'>Vintage Camera Rental</router-link>
		          <div class="date"><span>03.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program283'>사진 작가가 직접 알려주는 Photo Class</router-link>
		          <div class="date"><span>03.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program284'>Film Camera</router-link>
		          <div class="date"><span>03.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program285'>Retro Camcorder</router-link>
		          <div class="date"><span>03.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program286'>Vintage Digital Camera</router-link>
		          <div class="date"><span>03.08</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program287'>3월의 덕콘</router-link>
		          <div class="date"><span>03.17</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program288'>3월의 덕콘 | 한로로</router-link>
		          <div class="date"><span>03.17</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program289'>3월의 덕콘 | 신인류</router-link>
		          <div class="date"><span>03.31</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program290'>Photo Class | 한기재 작가</router-link>
		          <div class="date"><span>03.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program291'>Photo Class | 동원 작가</router-link>
		          <div class="date"><span>04.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program292'>Photo Class | 동원 작가</router-link>
		          <div class="date"><span>05.12</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program293'>3월의 콘썰트</router-link>
		          <div class="date"><span>03.18</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program294'>3월의 콘썰트 | 성적표의 김민영</router-link>
		          <div class="date"><span>03.18</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program295'>3월의 콘썰트 | 십개월의 미래</router-link>
		          <div class="date"><span>03.25</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program296'>3월의 콘썰트 | 성덕</router-link>
		          <div class="date"><span>04.01</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program297'>홈레코딩 입문</router-link>
		          <div class="date"><span>03.21</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program298'>LP 청음 입문</router-link>
		          <div class="date"><span>03.23</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program299'>기타 프레이즈 데일리 클래스</router-link>
		          <div class="date"><span>03.22,03.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program300'>나만의 커스텀 휴대폰 만들기</router-link>
		          <div class="date"><span>03.21,03.22,03.29</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program301'>에이닷으로 사진편집하기</router-link>
		          <div class="date"><span>03.22</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program302'>아이폰으로 gif만들기</router-link>
		          <div class="date"><span>03.21,03.29</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program304'>애플워치 100% 활용하기 꿀팁</router-link>
		          <div class="date"><span>03.21,03.28</span></div>
	          </li>
            <li class="pagelist-item">
		          <router-link class="link" to='/experience/program305'>갤럭시 & 아이폰 비하인드</router-link>
		          <div class="date"><span>03.22,03.27</span></div>
	          </li>

          </ul>

          <h2 class="pagelist-title">pay</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/paylogin'>구매 로그인 (확인용)</router-link>
              <div class="date"><span>3.19</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/pay'>구매 상세</router-link>
              <div class="date"><span>6.5</span><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/pay/complete'>구매 완료</router-link>
              <div class="date"><span>3.19</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/pay/complete/vm'>구매 완료 - 밴딩머신</router-link>
              <div class="date"><span>3.19</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/pay/fail'>결제 실패</router-link>
              <div class="date"><span>6.10</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">sev</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/program'>프로그램 예약</router-link>
              <div class="date"><span>3.17</span><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/experience'>체험존 예약</router-link>
              <div class="date"><span>3.17</span><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling'>상담 예약</router-link>
              <div class="date"><span>3.17</span><span>6.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/application'>프로그램 신청</router-link>
              <div class="date"><span>4.5</span><span>21.4.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/applicationSurvey'>프로그램 신청 - 기초 설문</router-link>
              <div class="date"><span>4.8</span><span>21.4.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/applicationComplete'>프로그램 신청 - 완료</router-link>
              <div class="date"><span>4.8</span><span>21.4.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/program/date/service'>예약일시 - 프로그램 (매장 선택)</router-link>
              <div class="date"><span>3.17</span><span>11.24</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/program/date/shop'>예약일시 - 프로그램 (프로그램 선택)</router-link>
              <div class="date"><span>3.17</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/experience/date/service'>예약일시 - 체험존 (매장 선택)</router-link>
              <div class="date"><span>3.17</span><span>6.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/experience/date/shop'>예약일시 - 체험존 (체험존 선택)</router-link>
              <div class="date"><span>3.17</span><span>6.11</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling/date/service'>예약일시 - 상담 예약</router-link>
              <div class="date"><span>3.17</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling/date/shop'>예약일시 - 상담 예약 (방문 목적 선택)</router-link>
              <div class="date"><span>3.17</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/program/info'>신청정보</router-link>
              <div class="date"><span>3.17</span><span>7.9</span></div>
            </li>
            <!-- li class="pagelist-item">
              <router-link class="link" to='/confirm/infologout'>신청정보 - 로그아웃 (확인용)</router-link>
              <div class="date"><span>3.17</span></div>
            </li -->
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling/survey'>기초 설문</router-link>
              <div class="date"><span>3.17</span><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling/surveycomplete'>기초 설문 완료상세</router-link>
              <div class="date"><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/standby/complete'>대기 접수 - 프로그램</router-link>
              <div class="date"><span>4.9</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/program/complete'>예약 완료 - 프로그램</router-link>
              <div class="date"><span>3.17</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/experience/complete'>예약 완료 - 체험존</router-link>
              <div class="date"><span>3.17</span><span>6.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/booking/counseling/complete'>예약 완료 - 상담</router-link>
              <div class="date"><span>3.17</span><span>7.9</span></div>
            </li>
            <!-- li class="pagelist-item">
              <router-link class="link" to='/sev/counselor'>상담원 호출</router-link>
              <div class="date"><span>3.17</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/counselorcomplete'>상담원 호출 - 배정완료 (확인용)</router-link>
              <div class="date"><span>3.17</span><span>4.9</span></div>
            </li -->
            <li class="pagelist-item">
              <router-link class="link" to='/sev/consult'>상담 신청하기</router-link>
              <div class="date"><span>6.5</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/consult/complete'>상담 신청 완료</router-link>
              <div class="date"><span>6.5</span><span>6.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/sev/faq'>이용안내</router-link>
              <div class="date"><span>3.17</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/confirm/faqnone'>이용안내 - list none (확인용)</router-link>
              <div class="date"><span>3.17</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">my</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/my/wish'>관심상품</router-link>
              <div class="date"><span>3.2</span><span>4.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/paylist'>구매 내역</router-link>
              <div class="date"><span>3.27</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/list'>예약/신청 내역</router-link>
              <div class="date"><span>3.27</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/applicationModify'>프로그램 신청 - 수정</router-link>
              <div class="date"><span>4.11</span><span>21.4.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/program/modify'>예약 수정 - 프로그램</router-link>
              <div class="date"><span>3.27</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/experience/modify'>예약 수정 - 체험존</router-link>
              <div class="date"><span>3.27</span><span>6.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/counseling/modify'>예약 수정 - 상담</router-link>k
              <div class="date"><span>3.27</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/counseling/survey'>기초 설문</router-link>
              <div class="date"><span>3.27</span><span>4.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/program/complete'>수정 완료 - 프로그램</router-link>
              <div class="date"><span>3.27</span><span>3.30</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/experience/complete'>수정 완료 - 체험존</router-link>
              <div class="date"><span>3.27</span><span>6.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/booking/counseling/complete'>수정 완료 - 상담</router-link>
              <div class="date"><span>3.27</span><span>7.9</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/checkin'>빠른 체크인</router-link>
              <div class="date"><span>3.23</span><span>6.5</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/modify'>빠른 체크인 수정</router-link>
              <div class="date"><span>3.23</span><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/register'>빠른 체크인 얼굴 등록</router-link>
              <div class="date"><span>8.21</span><span>10.12</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/coupon'>쿠폰 관리</router-link>
              <div class="date"><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/my/satisfaction'>만족도 조사</router-link>
              <div class="date"><span>21.4.21</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">shop</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/list'>매장 찾기</router-link>
              <div class="date"><span>4.3</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail'>매장 상세</router-link>
              <div class="date"><span>4.3</span><span>1.8</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">스토어 상세</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail?store_id=19868'>T Factory를 소개합니다</router-link>
              <div class="date"><span>01.15</span><span>01.29</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store2'>Factory Gate.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store3'>FLEX Stage.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store4'>Bestsellers.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store5'>Security Pod.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store6'>Media Pod.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store7'>0 Favorite.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store8'>Media Zone.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store9'>Factory Garden.</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store10'>Consulting Lounge</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store11'>11 STREET</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store12'>Data Station</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store13'>Apple Zone</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store14'>0 Stage</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store15'>Accessory Zone</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store16'>T1</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store17'>5GX Cloud Game</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store18'>Giant Face</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store19'>T Factory 24</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/shop/detail/store20'>크루 투어</router-link>
              <div class="date"><span>01.15</span><span>01.18</span></div>
            </li>
          </ul>

          <h2 class="pagelist-title">event</h2>
          <ul>
            <li class="pagelist-item">
              <router-link class="link" to='/event/list'>목록</router-link>
              <div class="date"><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/event/view/html'>상세 : HTML</router-link>
              <div class="date"><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/event/view/imagebutton'>상세 : 이미지 + 버튼</router-link>
              <div class="date"><span>8.21</span></div>
            </li>
            <li class="pagelist-item">
              <router-link class="link" to='/event/view/imagetext'>상세 : 이미지 + 텍스트</router-link>
              <div class="date"><span>8.21</span></div>
            </li>
          </ul>
        </div>
      </main>
    </div>
  </div>
</template>

<script>
export default {
  name: 'pagelist'
}
</script>

<style scoped>
  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 100;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Thin.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Thin.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Thin.otf) format("opentype")
  }

  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 300;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Light.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Light.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Light.otf) format("opentype")
  }

  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 400;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Regular.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Regular.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Regular.otf) format("opentype")
  }

  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 500;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Medium.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Medium.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Medium.otf) format("opentype")
  }

  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 700;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Bold.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Bold.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Bold.otf) format("opentype")
  }

  @font-face {
    font-family: 'Noto Sans KR';
    font-style: normal;
    font-weight: 900;
    src: url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Black.woff2) format("woff2"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Black.woff) format("woff"), url(https://fonts.gstatic.com/ea/notosanskr/v2/NotoSansKR-Black.otf) format("opentype")
  }

  /* nomalrize */
  body {
    overflow-y: scroll;
    overflow-x: auto;
    min-width: 320px;
    font-size: 12px;
    color: #000;
    background: transparent
  }

  html, body {
    position: relative;
    height: 100%
  }

  article, aside, details, figcaption, figure, footer, header, hgroup, menu, nav, section {
    display: block
  }

  body, p, h1, h2, h3, h4, h5, h6, ul, ol, li, dl, dt, dd, table, thead, tbody, tr, th, td, form, fieldset, legend, input, textarea, button, select, option {
    margin: 0;
    padding: 0;
    font-family: 'Noto Sans KR', sans-serif, Arial, Verdana;
    font-weight: 600;
    word-break: keep-all;
    -webkit-text-size-adjust: none;
    box-sizing: border-box
  }

  img, fieldset, button, frame {
    vertical-align: middle;
    border: 0
  }

  ul, ol {
    list-style: none
  }

  em, address {
    font-style: normal
  }

  hr {
    display: none;
    visibility: hidden
  }

  caption {
    visibility: hidden;
    overflow: hidden;
    top: 0;
    left: 0;
    width: 0;
    height: 0;
    font-size: 0;
    line-height: 0
  }

  input, textarea, button, select {
    vertical-align: middle;
    -webkit-tap-highlight-color: transparent;
    outline: 0
  }

  input[type=text],
  select {
    -webkit-appearance: none;
    -moz-appearance: none;
    appearance: none
  }

  input[type=submit] {
    margin: 0;
    padding: 0;
    border: 0;
    cursor: pointer
  }

  label {
    cursor: pointer;
    -webkit-tap-highlight-color: transparent
  }

  button {
    border: 0;
    outline: 0;
    vertical-align: middle;
    cursor: pointer;
    overflow: visible;
    background: transparent
  }

  button::-moz-focus-inner {
    border: 0;
    padding: 0
  }

  a {
    text-decoration: none;
    color: #000;
    -webkit-tap-highlight-color: transparent
  }

  a:hover, a:active, a:focus {
    text-decoration: none
  }

  @media screen and (-webkit-min-device-pixel-ratio: 0) {
    select, textarea, input {
      font-size: 16px
    }
  }

  /* lib */
  .swiper-container-horizontal > .swiper-pagination-bullets {
    bottom: 20px
  }

  .swiper-container-horizontal > .swiper-pagination-bullets .swiper-pagination-bullet {
    margin: 2px 5px;
    background: #d8d8d8;
    opacity: 1
  }

  .swiper-container-horizontal > .swiper-pagination-bullets .swiper-pagination-bullet-active {
    margin-top: 0;
    margin-bottom: 0;
    width: 7px;
    height: 7px;
    background: #fff;
    border: 2px solid #e98e42
  }

  .app__DT .swiper-container-horizontal > .swiper-pagination-bullets .swiper-pagination-bullet-active {
    border-color: #c93d8d
  }

  /* common */
  .blind {
    position: absolute;
    width: 1px;
    height: 1px;
    margin: -1px;
    padding: 0;
    border: 0;
    overflow: hidden;
    text-indent: -5000em
  }

  .text__point {
    color: #e98e42
  }

  .app__DT .text__point {
    color: #c93d8d
  }

  .text__warning {
    color: #ff0000
  }

  /*.icon{display:block;background:url(../image/icons@2x.png) no-repeat -999em -999em;background-size:200px auto}*/
  .icon-camera {
    width: 12px;
    height: 11px;
    background-position: -140px -60px
  }

  .icon-checkin {
    width: 42px;
    height: 42px;
    background-position: 0 -90px
  }

  .app__DT .icon-checkin {
    background-position-y: -135px
  }

  .icon-close {
    width: 20px;
    height: 20px;
    background-position: -25px -60px
  }

  .icon-delete {
    width: 24px;
    height: 24px;
    background-position: -50px -60px
  }

  .icon-delete-key {
    width: 26px;
    height: 18px;
    background-position: -155px -60px
  }

  .icon-arrow {
    width: 24px;
    height: 24px;
    background-position: -80px -60px
  }

  .icon-qrcode {
    width: 26px;
    height: 26px;
    background-position: -110px -60px
  }

  .icon-check {
    width: 23px;
    height: 16px;
    background-position: -95px -135px
  }

  .app__DT .icon-check {
    background-position: -120px -135px
  }

  .button__default {
    width: 100%;
    height: 48px;
    border-radius: 24px;
    background-color: #cdcdcd;
    color: #fff;
    font-size: 16px
  }

  .button__default.button-active {
    background-color: #e98e42
  }

  .app__DT .button__default.button-active {
    background-color: #c93d8d
  }

  .button__avatar-setting {
    position: relative
  }

  .button__avatar-setting .image__avatar-thumb {
    width: 68px;
    height: 68px;
    border-radius: 50%;
    border: 4px solid #ededf2
  }

  .button__avatar-setting .box__radious {
    position: absolute;
    right: 1px;
    bottom: 1px;
    display: block;
    padding: 8px;
    width: 12px;
    height: 12px;
    background: #868891;
    border-radius: 50%
  }

  /* layout */
  #wrapper::before {
    content: '';
    position: fixed;
    display: block;
    width: 100%;
    height: 100%;
    background-position: 50% 0;
    background-size: 100% auto
  }

  #header {
    position: relative;
    padding: 20px 20px 0
  }

  #header:after {
    content: '';
    display: block;
    clear: both
  }

  #header .title {
    display: block;
    font-size: 20px;
    line-height: 1
  }

  #header .status {
    text-align: right;
    font-size: 12px;
  }

  .page__main #header .title {
    font-size: 36px;
    line-height: 54px;
    font-weight: 800
  }

  #header .button__prev {
    display: block;
    float: left;
    margin-right: 12px;
    width: 20px;
    height: 20px
  }

  /*#header .button__prev::before{content:'';display:block;width:20px;height:20px;background:url(../image/icons@2x.png) no-repeat 0 -60px;background-size:200px auto}*/
  #container {
    position: relative;
    margin: 0 20px
  }

  #navigation {
    position: fixed;
    bottom: 0;
    left: 0;
    right: 0;
    z-index: 10;
    height: 74px;
    text-align: center;
    transform: translateY(0px);
    transition: all 0.5s;
    transition-timing-function: cubic-bezier(.5, -0.02, 0, .99);
    -webkit-transform: translateY(0px);
    -webkit-transition: all 0.5s;
    -webkit-transition-timing-function: cubic-bezier(.5, -0.02, 0, .99);
    background-size: 100% auto
  }

  #navigation .button__trip,
  #navigation .button__wallet,
  #navigation .button__myid {
    box-sizing: border-box;
    display: block;
    float: left;
    padding: 15px 0 12px;
    width: 33.33%;
    height: 100%;
    color: rgba(153, 153, 153, .46);
    font-size: 10px
  }

  #navigation .button-active {
    color: #000
  }

  #navigation .button__trip::before,
  #navigation .button__wallet::before,
    /*#navigation .button__myid::before{content:'';display:block;margin:0 auto 4px;width:28px;height:28px;background:url(../image/icons@2x.png) no-repeat;background-size:200px auto;opacity:1}*/
  #navigation .button__trip::before {
    background-position: 0 0
  }

  #navigation .button__wallet::before {
    background-position: -30px 0
  }

  #navigation .button__myid::before {
    background-position: -60px 0
  }

  #navigation .button__trip.button-active::before {
    background-position: 0 -30px
  }

  #navigation .button__wallet.button-active::before {
    background-position: -30px -30px
  }

  #navigation .button__myid.button-active::before {
    background-position: -60px -30px
  }

  #wrapper.scroll-down #navigation {
    -webkit-transform: translateY(100%);
    transform: translateY(100%)
  }

  /* intro */
  .service__intro {
    height: 100%
  }

  .service__intro #wrapper {
    height: 100%
  }

  .service__intro #container {
    margin: 0;
    padding: 0;
    height: 100%
  }

  .box__intro {
    width: 100%;
    height: 100%
  }

  .box__intro .swiper-slide {
    text-align: center;
    background: #fff;
    display: -webkit-box;
    display: -ms-flexbox;
    display: -webkit-flex;
    display: flex;
    -webkit-box-pack: center;
    -ms-flex-pack: center;
    -webkit-justify-content: center;
    justify-content: center;
    -webkit-box-align: center;
    -ms-flex-align: center;
    -webkit-align-items: center;
    align-items: center
  }

  .box__intro .intro__title {
    position: relative;
    display: flex;
    -webkit-box-pack: center;
    -ms-flex-pack: center;
    -webkit-justify-content: center;
    justify-content: center;
    -webkit-box-align: center;
    -ms-flex-align: center;
    -webkit-align-items: center;
    align-items: center;
    color: #000;
    font-size: 20px;
    line-height: 28px;
    padding-top: 222px;
    width: 100%;
    height: 56px
  }

  .box__intro .intro__title::before {
    content: '';
    position: absolute;
    top: 0;
    left: 50%;
    width: 180px;
    height: 180px;
    background-size: 180px auto;
    transform: translateX(-50%)
  }

  .box__intro .box__intro-01,
  .box__intro .box__intro-02,
  .box__intro .box__intro-03,
  .box__intro .box__intro-04 {
    height: 420px
  }

  /*.box__intro .box__intro-01 .intro__title::before{background-image:url(../image/intro-01@2x.png)}*/
  /*.box__intro .box__intro-02 .intro__title::before{background-image:url(../image/intro-02@2x.png)}*/
  /*.box__intro .box__intro-03 .intro__title::before{background-image:url(../image/intro-03@2x.png)}*/
  /*.box__intro .box__intro-04 .intro__title::before{background-image:url(../image/intro-04@2x.png)}*/
  .box__intro .intro__contents {
    margin-top: 12px;
    color: #333;
    font-size: 13px;
    line-height: 24px;
    font-weight: 400
  }

  .box__intro .button__default {
    position: absolute;
    left: 20px;
    right: 20px;
    bottom: 54px;
    display: block;
    width: auto;
    line-height: 48px;
    font-weight: 600
  }

  /* Sign-in/Join */
  /*.service__join #wrapper::before{background-image:url(../image/bg-wrapper@2x.png)}*/
  .box__pin {
    overflow: hidden;
    position: relative;
    text-align: center;
    display: flex;
    justify-content: center;
    align-items: center;
    flex-direction: column
  }

  .box__pin .text__pin {
    margin-bottom: 34px;
    font-size: 18px;
    line-height: 26px
  }

  .box__pin .pin__input {
    position: relative;
    margin: 0 auto 120px;
    height: 28px;
    width: 280px
  }

  .box__pin .pin__input .input__hidden {
    position: absolute;
    left: 0;
    top: 0;
    right: 0;
    bottom: 0;
    z-index: 10;
    width: 100%;
    height: 100%;
    background: transparent;
    border: 0 none;
    color: transparent
  }

  .os__ios .box__pin .pin__input .input__hidden {
    font-size: 0
  }

  .box__pin .pin__input .button__pin-number {
    width: 100%;
    height: 100%
  }

  .box__pin .pin__input .pin {
    display: inline-block;
    margin: 0 13px;
    width: 22px;
    height: 22px;
    border-radius: 14px;
    background: #c9c9c9
  }

  .box__pin .pin__input .pin-active {
    background-color: #e98e42
  }

  .app__DT .box__pin .pin__input .pin-active {
    background-color: #c93d8d
  }

  .box__pin .box__fly {
    position: absolute;
    bottom: 0
  }

  .box__pin .box__fly .text__warning {
    margin: 14px auto;
    width: 100%;
    font-size: 12px;
    font-weight: 400;
    text-align: center
  }

  .box__pin .box__fly .button__reset {
    display: block;
    margin: 14px auto;
    color: #868891;
    line-height: 1.5;
    border-bottom: 2px solid #868891
  }

  .box__keypad {
    position: fixed;
    left: 0;
    bottom: -1px;
    right: 0;
    background: #edeef1
  }

  .box__keypad .button__key {
    display: block;
    float: left;
    width: 33.33%;
    height: 60px;
    border: 1px solid #fff;
    border-width: 0 0 1px 1px;
    font-size: 24px
  }

  .box__keypad .button__key .icon-delete-key {
    display: inline-block
  }

  .box__issue .input__text,
  .box__issue .input__select {
    background: #fff;
    border: 1px solid #e9e9e9;
    border-radius: 26px;
    width: 100%;
    height: 46px;
    text-indent: 24px;
    font-size: 12px
  }

  .box__issue .input__select option {
    color: #000
  }

  .box__issue .input-focus {
    border: 2px solid #e98e42;
    text-indent: 23px
  }

  .app__DT .box__issue .input-focus {
    border-color: #c93d8d
  }

  .box__issue .input__text::placeholder,
  .box__issue .input__select:invalid,
  .box__issue .input__select option[value=""] {
    color: #d9d9d9;
    font-size: 12px;
    font-weight: 400;
    line-height: 25px
  }

  .box__issue .text__warning {
    padding: 7px 22px;
    font-size: 12px;
    font-weight: 400;
    line-height: 16px
  }

  .box__issue .button__input-delete {
    display: none;
    position: absolute;
    right: 10px;
    top: 11px;
    width: 24px;
    height: 24px
  }

  .box__issue .issue-active .input-focus + .button__input-delete {
    display: block
  }

  .box__issue .layer__suggest {
    display: none;
    position: absolute;
    left: 0;
    right: 0;
    top: 46px;
    z-index: 1;
    border-radius: 0 0 26px 26px;
    border: 2px solid #e98e42;
    background: #fff
  }

  .app__DT .box__issue .layer__suggest {
    border-color: #c93d8d
  }

  .box__issue .layer__suggest .box__scroll {
    position: relative;
    padding: 4px 24px 5px;
    max-height: 207px;
    overflow-y: auto
  }

  .box__issue .layer__suggest .button__suggest {
    padding: 10px 0;
    width: 100%;
    font-size: 12px;
    text-align: left
  }

  .box__issue .issue-active .input__suggest.input-focus {
    border-radius: 26px 26px 0 0;
    border: 2px solid #e98e42;
    border-bottom: 0 none
  }

  .app__DT .box__issue .issue-active .input__suggest.input-focus {
    border-color: #c93d8d
  }

  .box__issue .issue-active .input-focus ~ .layer__suggest {
    display: block
  }

  .box__issue .issue-active .input-focus + .button__input-arrow {
    display: block
  }

  .box__issue [class^=issue__] {
    position: relative
  }

  .box__issue .issue__avatar .button__avatar-setting {
    display: block;
    margin: 0 auto
  }

  .box__issue .issue__name {
    margin-top: 16px
  }

  .box__issue .issue__gender {
    margin-top: 10px
  }

  .box__issue .issue__gender.issue-active .input__suggest.input-focus {
    border: 0 none
  }

  .box__issue .issue__gender .layer__suggest {
    top: -2px;
    border-radius: 26px
  }

  .box__issue .issue__gender.issue-active .icon-arrow {
    transform: rotate(180deg)
  }

  .box__issue .issue__nationality {
    margin-top: 10px
  }

  .box__issue .issue__nationality .layer__suggest {
    border-top: 0 none
  }

  .box__issue .issue__email {
    margin-top: 10px
  }

  .box__issue .issue__organization {
    margin-top: 10px
  }

  .box__issue .button__input-arrow {
    position: absolute;
    right: 10px;
    top: 11px;
    z-index: 10
  }

  .box__issue .button__input-arrow {
    width: 24px;
    height: 24px
  }

  .box__issue .button__default {
    margin-top: 34px
  }

  .box__issue .button__issue-terms {
    display: block;
    margin: 14px auto;
    color: #868891;
    line-height: 1.5;
    border-bottom: 2px solid #868891
  }

  /* Trip */
  .service__trip.page__main #container {
    margin: 0
  }

  .service__trip.page__main #navigation {
    background-color: #fff
  }

  .box__trip {
    margin: 0 auto;
    max-width: 480px
  }

  .box__trip-card {
    position: relative
  }

  /*.box__trip-card:after{content:'';position:absolute;right:42px;top:23%;z-index:2;display:block;width:46px;height:17px;background:url(../image/icons@2x.png) no-repeat -45px -135px;background-size:200px auto}*/
  /*.box__trip-card.card-sk{margin-top:6px;background:url(../image/bg-box-trip@2x.png) no-repeat;background-size:100% auto}*/
  /*.box__trip-card.card-dt{margin-top:-6px;background:url(../image/bg-box-trip-dt@2x.png) no-repeat;background-size:100% auto}*/
  /*.box__trip-card.box__trip-checkin.card-sk{background:url(../image/bg-box-trip-checkin@2x.png) no-repeat;background-size:100% auto}*/
  /*.box__trip-card.box__trip-checkin.card-dt{background:url(../image/bg-box-trip-dt-checkin@2x.png) no-repeat;background-size:100% auto}*/
  .box__trip-card .image__trip-frame {
    width: 100%
  }

  .box__trip-card .label {
    color: rgba(0, 0, 0, 0.3);
    font-size: 10px
  }

  .box__trip-card .text__value {
    color: #fff;
    font-size: 12px;
    line-height: 18px
  }

  .box__trip-card .trip__entering .text__value {
    font-size: 18px;
    line-height: 28px;
    font-weight: 900
  }

  .box__trip-card .trip__entering,
  .box__trip-card .trip__location,
  .box__trip-card .trip__passenger {
    position: absolute;
    left: 42px
  }

  .box__trip-card .trip__entering {
    top: 9%
  }

  .box__trip-card .trip__location {
    bottom: 37%
  }

  .box__trip-card .trip__passenger {
    bottom: 14%
  }

  .box__trip-card .trip__location .label,
  .box__trip-card .trip__passenger .label {
    display: block;
    margin-bottom: 4px
  }

  .box__trip .button__learnmore {
    position: absolute;
    right: 20px;
    top: -40px;
    text-align: right;
    color: #bbb;
    font-size: 12px;
    line-height: 14px;
    letter-spacing: -0.3px
  }

  .service__trip.page__pavilion #container {
    margin: 0
  }

  .service__trip.page__pavilion #navigation {
    background-color: #fff
  }

  /* .service__trip.page__entrance #wrapper::before{background-image:url(../image/bg-wrapper@2x.png)}
  .service__trip.page__entrance #navigation{background-image:url(../image/bg-wrapper@2x.png)} */
  .service__trip.page__entrance #navigation {
    background-color: #fff
  }

  .box__entrance {
    padding-top: 24px
  }

  .box__entrance .text__entrance-title {
    font-size: 14px;
    line-height: 26px;
    text-align: center
  }

  .box__entrance .text__entrance {
    margin-top: 3px;
    padding-bottom: 13.5px;
    color: #999;
    font-size: 10px;
    font-weight: 400;
    line-height: 16px;
    text-align: center
  }

  .box__entrance .entrance__list {
    position: relative;
    padding: 17px 0;
    border-top: 1px solid #e7e7e7
  }

  .box__entrance .entrance__list .label,
  .box__entrance .entrance__list .text__value {
    display: block
  }

  .box__entrance .entrance__list .label {
    color: #cdcdcd;
    font-size: 10px
  }

  .box__entrance .entrance__list .text__value {
    font-size: 14px;
    color: #000
  }

  .box__entrance .entrance__list .icon-check {
    position: absolute;
    right: 5px;
    top: 50%;
    transform: translateY(-50%)
  }

  .box__entrance .entrance__list .input__switch {
    position: absolute;
    display: block;
    right: 0;
    top: 50%;
    transform: translateY(-50%)
  }

  .box__entrance .entrance__list .input__switch input[type=checkbox] {
    position: absolute;
    opacity: 0
  }

  .box__entrance .entrance__list .input__switch .input__switch-icon {
    position: relative;
    display: inline-block;
    margin-top: -2px;
    margin-right: 3px;
    vertical-align: middle;
    width: 70px;
    height: 40px;
    background-color: #d8d8d8;
    border-radius: 26px;
    -webkit-transition: background-color 0.4s;
    transition: background-color 0.4s
  }

  .box__entrance .entrance__list .input__switch .input__switch-icon:after {
    position: absolute;
    content: "";
    top: 3px;
    bottom: 3px;
    left: 3px;
    width: 34px;
    background-color: #fff;
    border-radius: 50%;
    box-shadow: 1px 0 3px rgba(0, 0, 0, 0.1);
    transition: transform 0.15s ease-out
  }

  .box__entrance .entrance__list .input__switch input[type=checkbox]:checked + .input__switch-icon {
    background-color: #e98e42
  }

  .app__DT .box__entrance .entrance__list .input__switch input[type=checkbox]:checked + .input__switch-icon {
    background-color: #c93d8d
  }

  .box__entrance .entrance__list .input__switch input[type=checkbox]:checked + .input__switch-icon:after {
    box-shadow: 2px 2px 12px 0 rgba(0, 0, 0, 0.08);
    transform: translateX(29px)
  }

  .box__entrance .entrance__list .input__switch .input__switch-status {
    position: absolute;
    left: 7px;
    top: 9px
  }

  .box__entrance .entrance__list .input__switch .input__switch-status:after {
    content: 'OFF';
    color: #d8d8d8;
    font-size: 12px;
    font-weight: 600
  }

  .box__entrance .entrance__list .input__switch input[type=checkbox]:checked ~ .input__switch-status {
    left: auto;
    right: 14px
  }

  .box__entrance .entrance__list .input__switch input[type=checkbox]:checked ~ .input__switch-status:after {
    content: 'ON';
    color: #e98e42
  }

  .app__DT .box__entrance .entrance__list .input__switch input[type=checkbox]:checked ~ .input__switch-status:after {
    color: #c93d8d
  }

  .box__entrance .entrance__confirm {
    padding-bottom: 20px;
    border-top: 1px solid #e7e7e7
  }

  .box__entrance .entrance__confirm .text__checkbox {
    padding: 20px 0 23px;
    text-align: right
  }

  .box__entrance .entrance__confirm .input__checkbox {
    position: absolute;
    opacity: 0
  }

  .box__entrance .entrance__confirm .label {
    position: relative;
    display: inline-block;
    color: #333;
    font-size: 13px;
    font-weight: 400
  }

  .box__entrance .entrance__confirm .label:before {
    content: '';
    display: inline-block;
    margin-right: 10px;
    width: 20px;
    height: 20px;
    border-radius: 4px;
    border: 2px solid #d7d4de;
    vertical-align: middle
  }

  .box__entrance .entrance__confirm .input__checkbox:checked + .label:before {
    border-color: #e98e42
  }

  .app__DT .box__entrance .entrance__confirm .input__checkbox:checked + .label:before {
    border-color: #c93d8d
  }

  /*.box__entrance .entrance__confirm .input__checkbox:checked+.label:after{content:'';position:absolute;left:1px;top:4px;display:block;display:block;background:url(../image/icons@2x.png) no-repeat -999em -999em;background-size:200px auto;width:20px;height:20px;width:23px;height:16px;background-position:-95px -135px;transform:scale(0.8)}*/
  .app__DT .box__entrance .entrance__confirm .input__checkbox:checked + .label:after {
    background-position: -120px -135px
  }

  .box__entrance .entrance__confirm .button__default {
    background: #e98e42
  }

  .app__DT .box__entrance .entrance__confirm .button__default {
    background: #c93d8d
  }

  .box__pavilion .pavilion__button {
    margin: 15px 20px 14px
  }

  .box__pavilion .pavilion__swipe-nav {
    position: relative;
    padding-left: 12px;
    height: 100px;
    background-color: rgba(226, 230, 235, 0.55);
    box-sizing: border-box
  }

  .box__pavilion .pavilion__swipe-nav .swiper-slide {
    position: relative;
    margin: 14px 8px;
    width: 72px;
    height: auto
  }

  .box__pavilion .pavilion__swipe-nav .swiper-slide::after {
    content: '';
    position: absolute;
    left: 0;
    top: 0;
    width: 66px;
    height: 66px;
    border: 3px solid transparent;
    border-radius: 50%
  }

  .box__pavilion .pavilion__swipe-nav .swiper-slide-thumb-active::after {
    border-color: #f47c25
  }

  .app__DT .box__pavilion .pavilion__swipe-nav .swiper-slide-thumb-active::after {
    border-color: #c93d8d
  }

  .box__pavilion .pavilion__swipe-nav .swiper-slide > img {
    border-radius: 50%;
    transform: scale(.93)
  }

  .box__pavilion .button__learnmore {
    position: absolute;
    right: 20px;
    top: -40px;
    text-align: right;
    color: #bbb;
    font-size: 12px;
    line-height: 14px;
    letter-spacing: -0.3px
  }

  .box__pavilion .text__pavilion-name {
    position: absolute;
    left: 0;
    top: 50%;
    width: 100%;
    color: #fff;
    transform: translateY(-50%);
    text-align: center;
    line-height: 14px
  }

  .box__pavilion .icon-checkin {
    position: absolute;
    right: -8px;
    bottom: -8px;
    z-index: 2
  }

  .box__pavilion .pavilion__contents-header {
    padding: 16px 20px;
    font-size: 16px
  }

  .box__pavilion .pavilion__contents-header .text__status {
    margin-top: 4px;
    font-size: 10px;
    color: #ff7a00
  }

  .app__DT .box__pavilion .pavilion__contents-header .text__status {
    color: #c93d8d
  }

  .box__pavilion .pavilion__contents-body .image__pavilion-content {
    width: 100%
  }

  .box__pavilion .pavilion__contents-body .text__pavilion-content {
    padding: 16px 20px;
    color: #333;
    font-size: 12px;
    font-weight: 400;
    line-height: 24px
  }

  .service__trip.page__booth {
    overflow: hidden
  }

  .service__trip.page__booth #container {
    margin: 0
  }

  .service__trip.page__booth #navigation {
    background-color: #fff
  }

  .box__booth {
    margin-top: 27px;
    overflow-y: auto;
  }

  .box__booth .booth__swipe .image__booth-content {
    width: 100%
  }

  .box__booth .booth__swipe .swiper-pagination-bullets {
    bottom: 18px;
    left: auto;
    right: 20px;
    width: auto
  }

  .box__booth .booth__swipe .swiper-pagination-bullets .swiper-pagination-bullet {
    margin: 1px 3px;
    background: #fff;
    opacity: 1
  }

  .box__booth .booth__swipe .swiper-pagination-bullets .swiper-pagination-bullet-active {
    margin-top: 0;
    margin-bottom: 0;
    width: 6px;
    height: 6px;
    background: transparent;
    border: 2px solid #fff
  }

  .box__booth .booth__contents {
    padding: 0 20px
  }

  .box__booth .booth__contents .text__booth-content {
    margin: 16px 0;
    color: #333;
    font-size: 12px;
    font-weight: 400;
    line-height: 24px
  }

  .box__booth .booth___fly {
    position: fixed;
    left: 0;
    right: 0;
    bottom: 74px;
    background: #fff
  }

  .scroll-down .box__booth .booth___fly {
    bottom: 10px
  }

  .box__booth .booth___fly .text__booth-message {
    margin: 0 20px;
    padding: 13px 0;
    color: #999;
    font-size: 10px;
    font-weight: 400;
    line-height: 16px;
    border-top: 1px solid #ddd
  }

  .box__booth .booth__button {
    margin: 0 20px
  }

  /*.service__trip.page__share #wrapper::before{background-image:url(../image/bg-wrapper@2x.png)}*/

  /* Wallet */
  /*.service__wallet.page__main #wrapper::before{background-image:url(../image/bg-wrapper-wallet@2x.png) }*/
  /*.service__wallet.page__main #navigation{background-image:url(../image/bg-wrapper-wallet@2x.png)}*/
  .service__wallet.page__main #container {
    margin: 0 20px
  }

  .service__wallet.page__main #header .title {
    color: #fff
  }

  .service__wallet.page__main #navigation .button__trip,
  .service__wallet.page__main #navigation .button__wallet,
  .service__wallet.page__main #navigation .button__myid {
    color: rgba(255, 255, 255, .46)
  }

  .service__wallet.page__main #navigation .button-active {
    color: #fff
  }

  .service__wallet.page__main #navigation .button__trip::before {
    background-position: -90px 0
  }

  .service__wallet.page__main #navigation .button__wallet::before {
    background-position: -120px 0
  }

  .service__wallet.page__main #navigation .button__myid::before {
    background-position: -150px 0
  }

  .service__wallet.page__main #navigation .button__trip.button-active::before {
    background-position: -90px -30px
  }

  .service__wallet.page__main #navigation .button__wallet.button-active::before {
    background-position: -120px -30px
  }

  .service__wallet.page__main #navigation .button__myid.button-active::before {
    background-position: -150px -30px
  }

  .box__wallet {
    position: relative;
    margin: 12px auto 0;
    max-width: 374px
  }

  .box__wallet .image__wallet-box {
    width: 100%
  }

  .button_qrcode {
    position: absolute;
    right: 0;
    top: -50px
  }

  .wallet__coins,
  .wallet__progress,
  .wallet__timeline-accumulation,
  .wallet__timeline-giveaways {
    position: absolute;
    left: 16px;
    right: 16px
  }

  .wallet__coins {
    top: 5%
  }

  .wallet__coins .text__coins {
    font-size: 10px;
    line-height: 14px
  }

  .wallet__coins .text__coins-total {
    position: absolute;
    right: 0;
    top: 0;
    font-size: 26px;
    line-height: 27px;
    font-weight: 900;
    text-align: right
  }

  .wallet__progress {
    top: 11%;
    margin: 12px 0;
    height: 4px;
    border-radius: 2px;
    background: #e0e6ec
  }

  .wallet__progress-status {
    height: 100%;
    background-image: linear-gradient(91deg, #6c2adc, #c05afe);
    border-radius: 2px
  }

  .wallet__timeline-accumulation,
  .wallet__timeline-giveaways {
    overflow-y: auto;
    right: 0
  }

  .wallet__timeline-accumulation {
    top: 17.5%;
    bottom: 31%
  }

  .wallet__timeline-giveaways {
    top: 74%;
    bottom: 3%
  }

  .wallet__timeline-accumulation .timeline-title {
    margin-bottom: 8px;
    color: #6385f5
  }

  .wallet__timeline-giveaways .timeline-title {
    margin-bottom: 8px;
    color: #f26a6a
  }

  .timeline-date {
    position: absolute
  }

  .timeline-contents {
    position: relative;
    margin-bottom: 10px;
    padding-left: 34px;
    font-weight: 400
  }

  .timeline-contents .text__timeline-coin {
    position: absolute;
    right: 16px;
    top: 0;
    color: #6385f5;
    font-weight: 600
  }

  .timeline-message {
    overflow: hidden;
    font-size: 10px;
    font-weight: 400;
    line-height: 16px;
    color: #999;
    text-align: center
  }

  .box__giveaways {
    padding-top: 88px
  }

  /*.giveaways__qrcode{margin:0 auto;padding:23px;width:100px;height:100px;background:url(../image/bg-box-giveaways-qrcode@2x.png) no-repeat;background-size:100% auto}*/
  .giveaways__contents {
    text-align: center
  }

  .giveaways__contents .text__username {
    margin: 18px 0 8px;
    font-size: 18px
  }

  .giveaways__contents .text__message {
    font-size: 12px;
    font-weight: 400;
    line-height: 18px
  }

  /* My ID */
  /*.service__myid #wrapper::before{background-image:url(../image/bg-wrapper@2x.png)}*/
  .service__myid #container {
    margin: 0 34px
  }

  /*.service__myid #navigation{background-image:url(../image/bg-wrapper@2x.png)}*/
  .box__myid {
    padding-top: 20px
  }

  .box__myid .myid__code {
    padding-bottom: 20px;
    margin-bottom: 30px;
    border-bottom: 1px solid #ddd
  }

  .box__myid .myid__profile {
    position: relative
  }

  /*.box__myid .myid__qrcode{margin-left:38%;padding:12px;width:60px;height:60px;background:url(../image/qr-code-popup@2x.png) no-repeat;background-size:100% auto}*/
  .box__myid .myid__title {
    position: absolute;
    color: #b3b3b3;
    font-size: 12px
  }

  .box__myid .myid__list {
    margin-bottom: 15px
  }

  .box__myid .myid__list:after {
    content: '';
    display: block;
    clear: both
  }

  .box__myid .myid__list .label,
  .box__myid .myid__list .text__value {
    float: left;
    line-height: 18px;
    box-sizing: border-box
  }

  .box__myid .myid__list .label {
    width: 38%;
    color: #cdcdcd;
    font-size: 10px
  }

  .box__myid .myid__list .text__value {
    color: #000
  }

  .box__myid .myid__list .button__edit {
    position: absolute;
    top: 26px;
    width: 60px;
    height: 30px;
    border-radius: 20px;
    background-color: #bbb;
    color: #fff
  }

  .box__myid .myid__list .image__avatar-thumb {
    display: block;
    padding-left: 38%;
    width: 76px;
    height: 76px
  }

  /* Modal */
  .freeze {
    overflow: hidden
  }

  .mask {
    position: fixed;
    top: 0;
    left: 0;
    z-index: 300;
    width: 100%;
    height: 100%;
    background: rgba(0, 0, 0, 0.6)
  }

  .box__modal {
    display: none;
    position: fixed;
    top: 0;
    bottom: 0;
    right: 20px;
    left: 20px;
    z-index: -999;
    margin: 0 auto;
    height: auto;
    opacity: 0;
    transform: translateY(100%);
    -webkit-transform: translateY(100%);
    outline: 0
  }

  .modal-active {
    z-index: 999;
    display: block;
    transform: none;
    opacity: 1
  }

  .modal__container {
    position: absolute;
    top: 50%;
    left: 0;
    right: 0;;
    transform: translateY(-50%);
    min-width: 280px;
    border-radius: 12px;
    background-color: #fff
  }

  .modal__contents {
    height: 100%
  }

  .box__modal .modal__body {
    padding: 50px 20px 30px;
    text-align: center
  }

  .box__modal .modal__body .text__title {
    margin-bottom: 18px;
    font-size: 20px;
    color: #000
  }

  .box__modal .modal__body .text__title::before {
    content: '';
    margin: 0 auto 22px;
    display: block;
    width: 120px;
    height: 120px;
    background-size: 120px auto
  }

  .box__modal .modal__body .text__contents {
    font-size: 13px;
    line-height: 20px;
    font-weight: 400;
    color: #666
  }

  .box__modal .modal__footer {
    padding: 0 40px 40px;
    text-align: right
  }

  .modal-alert .modal__body {
    padding: 36px 28px 20px;
    text-align: left
  }

  .modal-alert .modal__body .text__contents {
    font-size: 16px;
    line-height: 26px;
    font-weight: 500;
    color: #333
  }

  .modal-alert .button__modal-cancel,
  .modal-alert .button__modal-confirm {
    min-width: 46px;
    margin-left: 30px;
    font-size: 24px;
    text-align: right
  }

  .modal-alert .button__modal-cancel {
    color: #d9d9d9
  }

  .modal-alert .button__modal-confirm {
    color: #e98e42
  }

  .modal-terms {
    top: 20px;
    bottom: 20px
  }

  .modal-terms .modal__header {
    padding: 56px 20px 20px;
    font-size: 20px
  }

  .modal-terms .modal__container {
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    transform: none
  }

  .modal-terms .modal__body {
    position: absolute;
    top: 100px;
    right: 0;
    bottom: 20px;
    left: 0;
    padding: 0 20px;
    overflow: hidden;
    overflow-y: auto;
    -webkit-overflow-scrolling: touch
  }

  .modal-terms .modal__body .text__contents {
    font-size: 13px;
    font-weight: 400;
    line-height: 24px;
    color: #666;
    text-align: justify
  }

  .modal-terms .modal__footer {
    position: absolute;
    right: 24px;
    top: 24px;
    padding: 0
  }

  .modal-learnmore .modal__body {
    padding-bottom: 20px
  }

  .modal-learnmore .learnmore__title-description {
    font-size: 12px
  }

  .modal-learnmore .learnmore__title {
    font-size: 20px
  }

  .modal-learnmore .learnmore__title:after {
    content: '';
    margin: 27px auto 26px;
    display: block;
    width: 120px;
    height: 120px;
    background-size: 120px auto
  }

  /*.modal-learnmore .box__trip-learnmore-01 .learnmore__title:after{background-image:url(../image/learnmore-01@2x.png)}*/
  /*.modal-learnmore .box__trip-learnmore-02 .learnmore__title:after{background-image:url(../image/learnmore-02@2x.png)}*/
  /*.modal-learnmore .box__trip-learnmore-03 .learnmore__title:after{background-image:url(../image/learnmore-03@2x.png)}*/
  /*.modal-learnmore .box__trip-learnmore-04 .learnmore__title:after{background-image:url(../image/learnmore-04@2x.png)}*/
  .modal-learnmore .learnmore__contents {
    padding-bottom: 40px;
    color: #333;
    font-size: 13px;
    font-weight: 400;
    line-height: 24px
  }

  .modal-learnmore .swiper-pagination-bullets {
    bottom: 0
  }

  .modal-learnmore .modal__footer {
    position: absolute;
    right: 24px;
    top: 24px;
    padding: 0
  }

  .modal-share .modal__header {
    padding: 59px 20px 7px;
    font-size: 30px;
    text-align: center
  }

  .modal-share .modal__body {
    padding: 0 30px 98px
  }

  .modal-share .text__message {
    height: 41px;
    font-size: 16px;
    text-align: center
  }

  .modal-share .button__share-facebook,
  .modal-share .button__share-twitter {
    position: relative;
    display: block;
    padding-left: 78px;
    width: 100%;
    height: 52px;
    border-radius: 30px;
    color: #fff;
    text-align: left;
    font-size: 18px
  }

  .modal-share .button__share-facebook {
    margin-top: 40px;
    background-color: #4267b2
  }

  .modal-share .button__share-twitter {
    margin-top: 20px;
    background-color: #1ba1f3
  }

  .modal-share .button__share-facebook::before,
    /*.modal-share .button__share-twitter::before{content:'';position:absolute;left:19px;top:10px;display:block;width:38px;height:38px;background:url(../image/icons@2x.png) no-repeat;background-size:200px auto}*/
  .modal-share .button__share-facebook::before {
    background-position: -83px -90px
  }

  .modal-share .button__share-twitter::before {
    background-position: -123px -90px
  }

  .modal-share .modal__footer {
    position: absolute;
    right: 24px;
    top: 24px;
    padding: 0
  }

  .modal-mycode .modal__header {
    padding: 69px 20px 0;
    font-size: 18px;
    text-align: center
  }

  .modal-mycode .modal__body {
    padding: 26px 20px 48px
  }

  /*.modal-mycode .modal__body .image__frame{margin:0 auto;padding:23px 24px 25px;width:158px;height:158px;background:url(../image/qr-code-popup@2x.png) no-repeat;background-size:100% auto}*/
  .modal-mycode .modal__body .image__frame img {
    width: 100%
  }

  .modal-mycode .modal__footer {
    position: absolute;
    right: 24px;
    top: 24px;
    padding: 0
  }

  .app__DT .modal-alert .button__modal-confirm {
    color: #c93d8d
  }

  .modal-alert .modal__footer {
    padding-bottom: 24px
  }

  .service__trip .modal-alert .button__modal-confirm {
    color: #ff7a00
  }

  .app__DT .service__trip .modal-alert .button__modal-confirm {
    color: #e20274
  }

  /*.modal-entrance .modal__body .text__title::before{background-image:url(../image/entrance@2x.png)}*/
  /*.service__trip .modal-entrance .modal__body .text__title::before{background-image:url(../image/entrance-skt@2x.png)}*/
  /*.modal-coin .modal__body .text__title::before{background-image:url(../image/coin@2x.png)}*/
  /*.service__trip .modal-coin .modal__body .text__title::before{background-image:url(../image/coin-skt@2x.png)}*/

  .pagelist-title {
    margin: 20px 0 0;
    padding: 5px;
    color: #fff;
    font-size: 14px
  }

  .pagelist-title:first-of-type {
    margin-top: 10px;
  }

  .app__SKT .pagelist-title {
    background-color: #e98e42
  }

  .app__DT .pagelist-title {
    background-color: #c93d8d
  }

  .button-skt,
  .button-dt {
    margin-top: -5px;
    padding: 5px 10px;
    float: right;
    color: #fff;
    background: #dcdcdc
  }

  .app__SKT .button-skt {
    background: #e98e42
  }

  .app__DT .button-dt {
    background: #c93d8d
  }

  #container {
    margin-bottom: 20px
  }

  .pagelist-item {
    display: flex;
    border-bottom: 1px solid #eee
  }

  .pagelist-item .link {
    /*display: block;*/
    flex-basis: calc(100% - 110px);
    padding: 10px;
    color: #000;
    font-size: 13px;
    font-weight: 500
  }

  .pagelist-item .link[href^="#"] {
    color: #eee
  }

  .pagelist-item .date {
    /*display: block;*/
    flex-basis: 110px;
    margin-left: auto;
    padding: 12px 0 10px;
    font-size: 0;
    color: #666;
  }

  .pagelist-item .date span {
    display: inline-block;
    position: relative;
    width: 50%;
    font-size: 12px;
    text-align: center;
  }

  .pagelist-item .date span:nth-of-type(2):after {
    content: '/';
    position: absolute;
    top: 0;
    left: 0;
    font-size: 12px;
  }
</style>
