...
export default {
  ...
  computed: {
    ...
    modalStyle () {
      return {
        top: this.type === 'full' ? 0 : this.position.top + 'px',
        left: this.type === 'full' ? 0 : this.position.left + 'px',
        width: this.type === 'full' ? '100%' : this.trueModalWidth + 'px',
        height: this.type === 'full' ? window.screen.height + 'px' : (this.isAutoHeight ? 'auto' : this.trueModalHeight + 'px')
      }
    }
  },
  watch: {
